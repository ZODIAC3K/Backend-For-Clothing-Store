...
async function createOrder(req, res, next) {
	    ...
	    stockDetail
	        ...
	        .then(async (stock_details) => {
                ...
                if(stock_details.length == 0){
                    Errors.push({ product: product_id, message: "Invalid Product" });
                }
	                if (stock_details.quantity < quantity_ordered[idx]) {
                        Errors.push({ product: product_id, message: "Product is out of stock" });

	                }
	           if (offer_used[idx] !== 'null' && offer_used[idx] !== undefined)  {
	                // console.log(offer_used[idx]);
	                OfferDetails.find(offer_used[idx]).then((offer) => {
	                    if (!offer) {
                            Errors.push({ offer_used: offer_used[idx], message: "Offer not found" });

	                    } else {
	                        if (offer.applicable_on.includes(product_id)) {
	                            if (offer.end_at < Date.now()) {
                                    Errors.push({ offer_used: offer_used[idx], message: "Offer is expired" });

	                            } else {
	                                each_product_amount =
	                                    stock_details.amount -
	                                    stock_details.amount * (offer.offer_discount / 100); // amount - (amount * offer_discount/100)
	                                product_ordered_amount =
	                                    each_product_amount * quantity_ordered[idx]; //  (quantity * amount)
	                                final_amount += product_ordered_amount; // total amount after applying offer on each product
	                            }
	                        } else {
                                Errors.push({ offer_used: offer_used[idx], message: "Offer is not applicable on this product" });
	                        }
	                    }
	                });
	            }else{
	                each_product_amount = stock_details.amount;
	                product_ordered_amount = each_product_amount * quantity_ordered[idx];
	                final_amount += product_ordered_amount; // total amount after applying offer on each product
	            }
	        });
	});
	...
	CouponDetails.findById(coupon_used).then( async (coupon_used) => {
	    if(coupon_used){
	        if(coupon_used.end_at < Date.now()){
                Errors.push({ coupon_used: coupon_used, message: "Coupon is expired" });
	        }else{
	            final_amount = final_amount - (final_amount * coupon_used.discount/100); // final_amount - (final_amount * coupon_discount/100)
	        }
	    }
	});
	...
	product_ordered.forEach((product_id,idx) => {
	    stockDetail.findOne({
	        ...
	        stock_details.quantity = stock_details.quantity - quantity_ordered[idx];
	        stock_details.save();
	    });
	});
    if(Errors.length > 0){
        ...
        return res.status(201).json({ message: 'Order is in Processed And Stock Updated successfully ', userOrder });
    }
